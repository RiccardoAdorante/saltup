...
@pytest.fixture
def yolo_anchors_based_with_temp_model():
...
def test_preprocess(yolo_anchors_based_with_temp_model):
    ...
    processed_image = yolo_anchors_based_with_temp_model.preprocess(image, target_height, target_width)
    
    # Check the shape of the processed image
    assert processed_image.shape == (1, target_height, target_width, 1)
    
    # Check the type of the processed image
    assert processed_image.dtype == np.float32

# Test postprocess method
def test_postprocess(yolo_anchors_based_with_temp_model):
    # Generate a random raw output tensor
    raw_output = np.random.uniform(-2, 12, (1, 1, 15, 20, 5, 9))
    
    # Define image dimensions
    image_height, image_width = 480, 640
    
    # Postprocess the raw output
    result = yolo_anchors_based_with_temp_model.postprocess(raw_output, image_height, image_width)
